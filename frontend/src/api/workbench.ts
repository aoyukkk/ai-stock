...
export const workbenchApi = {
  ...
  resumeJob: (jobId: string) => apiPost<PipelineJob>(`/api/workbench/jobs/${encodeURIComponent(jobId)}/resume`),
  ...
  fundamentals: (tradeDate: string, pipelineRunId?: string | null, signal?: AbortSignal) => apiGet<{ items: Record<string, unknown>[]; currentness?: Record<string, unknown> }>(`/api/workbench/fundamentals/results?${dateQuery(tradeDate)}${pipelineRunId ? `&pipeline_run_id=${encodeURIComponent(pipelineRunId)}` : ""}`, { signal }),
